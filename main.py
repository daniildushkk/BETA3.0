...
class TextTranslator:
    ...
    async def translate_text(self, text: str, target_lang: str = 'en') -> str:
        """Перевод текста с помощью MyMemory API (бесплатно)"""
        ...
        try:
            params = {
                ...
                'de': 'your-email@example.com'  # Для лучшего качества перевода
            }
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.api_url, params=params, timeout=30) as response:
                    if response.status == 200:
                        ...
                        logger.warning(f"Translation API error: {response.status}")
                        return text
...
class VKParser:
    ...
    # Методы extract_title, extract_date, extract_time, extract_location остаются без изменений
...
async def send_event_message(chat_id, event_data, lang='ru'):
    ...
    event_text = translator.get_text('event_format', lang).format(
        ...
        date=formatted_date,
        time=event_time,
        time_preposition=time_preposition,
        location=location,
...
async def help_handler(message: Message):
    ...
    if len(VK_EVENT_KEYWORDS) > 5:
        keywords_text += f"\n• ... {translator.get_text('and_more', lang, count=len(VK_EVENT_KEYWORDS)-5)}" if lang == 'en' else f"\n• ... и еще {len(VK_EVENT_KEYWORDS)-5} слов"
